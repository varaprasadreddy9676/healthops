...
- `backend/config/default.json` is the **first-run seed** for runtime defaults and any starter checks. After the first run, MongoDB is the single source of truth — checks are managed via the API/UI and survive restarts. Edits to `default.json` are ignored once state exists.
- `backend/data/` stores AI encryption keys and JWT secrets.
